...
void main() async {
  ...
  idUser = CacheHelper.getData(key: "reId");
  print("the features is ${CacheHelper.getData(key: "benefit_${[7]}")}");
  userAccountType = CacheHelper.getData(key: "userAccountType");
  Constant.token = CacheHelper.getData(key: "retToken");
  print("the token is ${CacheHelper.getData(key: "retToken")}");
  print("the token is ${CacheHelper.getData(key: "price")}");
  print("the id is ${CacheHelper.getData(key: "reId")}");
  print(
      "the userAccountType is ${CacheHelper.getData(key: "userAccountType")}");
// print("id ${id!.toInt()}");
  runApp(SakenyApp());
}
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        ...
        BlocProvider(
          create: (context) =>
              UserDataCubit(ServicesLocator.getIt.get<HomeRepoImplementation>())
                ..getUserData(),
        ),
        BlocProvider(
        ...
        BlocProvider(
          create: (context) => GetPostCubit()..getFavorite()..getDataSuggest(),
        ),
        BlocProvider(
          create: (context) => PersonInformationCubit()
        ),
        BlocProvider(
            create: (context) => ChatCubit()
        ),
        BlocProvider(
            create: (context) => FeedBackUserCubit()
        ),
        BlocProvider(
      ...
      child: ScreenUtilInit(
        ...
        builder: (context, child) {
          return GetMaterialApp(
            ...
            initialRoute: CacheHelper.getData(key: "reId") != null
                ? (CacheHelper.getData(key: "userAccountType") == "مستأجر" ? "nav_tenant" : "nav")
                : "splash",
          );
        },
      ),
