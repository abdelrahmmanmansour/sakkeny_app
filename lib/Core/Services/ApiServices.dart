...
class ApiServices {
  getApi() async {
    http.Response response = await http.get(
        Uri.parse("http://sakenyapi.runasp.net/api/users/${idUser!.toInt()}"),
        headers: {
  ...
  Future<dynamic> putApi(Map<String, dynamic> body, String userId) async {
    try {
      final String url =
          "http://sakenyapi.runasp.net/api/users/$userId"; // Assuming userID needs to be part of the URL
      http.Response response = await http.put(Uri.parse(url),
          body: jsonEncode(body),
          headers: {"Authorization": "Bearer ${CacheHelper.getData(key: "retToken") }"});
